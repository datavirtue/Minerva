 ...
 *  setValueAt() methods.
 * 
 * USE: BeanTableModel model = new BeanTableModel(Class.class)
 * 
 */
public class BeanTableModel<T> extends RowTableModel<T>
{
	 ...
	 *  model.
	 */
	//private ArrayList<ColumnMap> colMaps = new ArrayList<ColumnMap>();
	...
	public Object getValueAt(int row, int column)
	{
       ...
       /* private int getPropertyColIndex(String s){
            if (colMaps == null) return -1;
            ColumnMap cm;
            for (int i = 0; i < colMaps.size(); i++){
                cm = colMaps.get(i);
                if (cm.getColumnName().equalsIgnoreCase(s)) return cm.getColumnIndex();
            }
            return -1;
        }*/
	...
	{
            ColumnInformation ci = (ColumnInformation)columns.get( getPropertyColIndex(columnName) );
...
class ColumnMap {
    ...
    public ColumnMap(String s, int i){
        this.columnName = s;
        this.columnIndex = i;
    }
