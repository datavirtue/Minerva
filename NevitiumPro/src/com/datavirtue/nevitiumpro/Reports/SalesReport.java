...
public SalesReport(AppSettings application, boolean vat){
...
private void appendItemToDaily(ArrayList itemList){
    ...
    for (int idx = 0; idx < itemList.size(); idx++){
        ...
        ii = (InvoiceItem)itemList.get(idx);
        item[0] = ii.getDate();
        //item[1] = ii.getCode();
        item[1] = ii.getQty();
        item[2] = ii.getDesc();
    ...
    for (long day = startDate; day <= endDate; day += oneDay){ //start day loop
        ...
        /* Remove the voids */
         ...
         for (int i = 0; i < temp.size(); i++){
            invoice = new InvoiceModel(application, (Integer)temp.get(i));            
        ...
        for (int i = 0; i < invoices.size(); i++){
            invoice = new InvoiceModel(application, (Integer)invoices.get(i));
